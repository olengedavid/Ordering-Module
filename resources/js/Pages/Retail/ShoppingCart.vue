<template>
            ...
            >
              ...
              <div class="cart-item-details">
                ...
                <div class="item-tag-container">
                  ...
                  <span v-if="item.outOfStock" class="item-tag out-of-stock">Out of stock</span>
                </div>
                
                <!-- Moved item-controls here, above the price -->
                <div class="item-controls-inline">
                  <div class="quantity-control">
                    <button @click="decreaseQuantity(index)" class="qty-btn decrease">−</button>
                    <span class="qty-value">{{ item.quantity }}</span>
                    <button @click="increaseQuantity(index)" class="qty-btn increase">+</button>
                  </div>
                </div>
                
                <div class="item-price">
                  <span class="price">Ksh {{ formatNumber(item.price * item.quantity) }}</span>
                  <span class="price-per-item">Ksh {{ formatNumber(item.pricePerItem) }} per item</span>
                </div>
                ...
                <div class="product-meta-grid">
                  <div class="meta-row">
                    <div class="product-meta-card">
                      ...
                      <div class="meta-card-content">
                        ...
                        <span class="meta-card-value">{{ item.minOrder }} item</span>
                      </div>
                    </div>
                    ...
                    <div class="product-meta-card">
                      ...
                      <div class="meta-card-content">
                        ...
                        <span class="meta-card-value">{{ item.maxOrder }} {{ item.maxOrderUnit }}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <button @click="removeItem(index)" class="remove-btn">Remove item</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="order-summary-container">
            <div class="order-summary">
              <div class="summary-section">
                <h3 class="section-title">Delivery Address <span class="required">*</span></h3>
                <textarea 
                  class="address-input"
              ...
              <div class="summary-section">
                <h3 class="section-title">Choose how to pay <span class="required">*</span></h3>
                <div class="payment-options">
                  ...
                  <label class="payment-option" :class="{ 'active': paymentMethod === 'mobile' }">
                    <input type="radio" v-model="paymentMethod" value="mobile" name="paymentMethod">
                    <span class="radio-label">Mobile Money</span>
                  </label>
                  
                  <label class="payment-option" :class="{ 'active': paymentMethod === 'credit' }">
                    <input type="radio" v-model="paymentMethod" value="credit" name="paymentMethod">
                    <span class="radio-label">Pay with Credit Facility</span>
                  </label>
                  <div class="balance-info">Available Balance: <span class="balance">Ksh {{ formatNumber(creditBalance) }}</span></div>
                </div>
              </div>
              ...
              <div class="fee-section">
                <div class="fee-row">
                  ...
                  <span class="fee-value">Ksh {{ formatNumber(deliveryFee) }}</span>
                </div>
              </div>
              ...
              <div class="total-section">
                <div class="total-row">
                  ...
                  <span class="total-value">Ksh {{ formatNumber(totalAmount) }}</span>
                </div>
              </div>
  ...
  export default {
    ...
    computed: {
      totalAmount() {
        return this.cartItems.reduce((total, item) => total + (item.price * item.quantity), 0) + this.deliveryFee;
      },
      canPlaceOrder() {
        return this.deliveryAddress.trim() !== '' && this.paymentMethod !== '';
      }
    },
    methods: {
      formatNumber(number) {
      ...
      increaseQuantity(index) {
        if (this.cartItems[index].quantity < this.cartItems[index].maxOrder && !this.cartItems[index].outOfStock) {
          this.cartItems[index].quantity++;
        }
      },
      decreaseQuantity(index) {
        if (this.cartItems[index].quantity > this.cartItems[index].minOrder) {
          this.cartItems[index].quantity--;
        }
      },
      removeItem(index) {
  ...
  .payment-option:hover, .payment-option input[type="radio"]:checked + .radio-label {
    border-color: #bbdefb;
    background-color: #f8fafd;
  }
