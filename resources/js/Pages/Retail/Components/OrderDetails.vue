    ...
    <div class="content-container">
      ...
      <div v-if="order" class="order-details-container">
        ...
        <div class="order-actions">
          <button class="action-button payment" v-if="showPayButton">
            Pay Now
          </button>
          <button
          ...
          <button class="action-button confirm" @click="showConfirmOrderDialog">
            Confirm Order
          </button>
          <button
            class="action-button delivered"
            @click="showMarkAsDeliveredDialog"
          >
            Mark as Delivered
          </button>
          <button
  ...
  <div v-if="dialog.show" class="dialog-overlay" @click="closeDialog">
    <div class="dialog-container" @click.stop>
      ...
      <div class="dialog-actions">
        <button class="dialog-btn secondary" @click="closeDialog">
          Cancel
        </button>
        <button
          :class="['dialog-btn', dialog.buttonType]"
          @click="confirmDialog"
        >
          {{ dialog.confirmText }}
        </button>
...
export default {
  ...
  methods: {
    ...
    confirmDialog() {
      if (
        this.dialog.action &&
        typeof this[this.dialog.action] === "function"
      ) {
    ...
    deleteItem(index) {
      ...
      const originalIndex = this.orderItems.findIndex(
        (i) =>
          i.name === item.name &&
      ...
      if (originalIndex !== -1) {
        ...
        // Update order total
        if (this.order) {
          this.order.amount = this.orderItems.reduce(
    ...
    async fetchOrder() {
      try {
        const response = await axios.get(route("retailer.orders.view"), {
